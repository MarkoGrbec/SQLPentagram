...
class Table:
	...
	func create_column(server: bool, data_base_dir: String, type: DataType, length: int, file_name_column: String, multi_array_length = 1):
		# change type
		var _file_name = DataBase.directory_exists(server, data_base_dir, _table_name, file_name_column)
		var header = DataBase.get_header(_file_name)
		var column_database = []
		# if type isn't same as meta
		if header and type != header[3]:
...
static func path(server : bool, DataBaseDir : String, tableNamePath : String):
	...
	if server:
		...
		# OS.get_user_data_dir() = /home/dusan
...
static func file_create_or_rea_or_write(file_path, file_mode : FileAccess.ModeFlags = FileAccess.ModeFlags.READ):
	...
	#region dir exists
static func directory_exists(server : bool, data_base_dir, table_name, column_name):
...
static func load_back_data(_path, length):
	...
	var column_name = ""
	#get meta
	var file_access = FileAccess.open(String("{file_name}").format({file_name = file_name}), FileAccess.READ)
...
static func get_header(file_name):
	...
	file_access.close()
	return [length, type, multi_array, original_type]
#endregion header
...
static func insert(server : bool, data_base_dir : String, table_name, column_name, id: int, data, _oper = "equals"):
	...
	if data is Array or data is PackedByteArray:
		if data.size() > length * meta_data[2]:
...
static func delete_column(server: bool, data_base_dir, table_name, column_name):
		...
		DirAccess.remove_absolute(String("{file_name}.meta").format({file_name = file_name}))
#endregion delete
...
#
	...
	func delete_row(idRow: int):
		...
		if mc:
			...
			DataBase.insert(true, dataBasePathText, table_name, "id", mc.id, 0, "equals")
