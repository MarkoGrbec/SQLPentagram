class_name NullList extends Node

#region NullList
func _init():
	_container = []
	_nullContainer = []
	_count = 1
	set_data(null)

var _container
var _nullContainer
var _count
func set_index_data(index, data):
	if _container.size() < index + 1:
		#we store nulls till the end of size
		for i in range(_count, index, 1):
			_nullContainer.push_back(i)
		_count = index + 1
		#it needs count
		_container.resize(_count)
	if _container[index] == null:
		for i in range(0, _nullContainer.size(), 1):
			if _nullContainer[i] == index:
				_nullContainer.remove_at(i)
				break
	_container[index] = data

func set_data(data):
	if _nullContainer.size() > 1:
		var index = _nullContainer.pop_back()
		_container[index] = data
		return index
	else:
		#just that it doesn't make full array nullable
		#if _container.size() == 0:
			#if data == null:
				#_container.push_back(0) # might lead to bug this workaround
				#return -1
		_container.push_back(data)
		_count = _container.size()
		return _count - 1

func get_index_data(index):
	if _count < index + 1:
		return null
	return _container[index]

## does not free the memory it only sets the data to null
func remove_at(index):
	if _count > index:
		_nullContainer.push_back(index)
		_container[index] = null
	else:
		_nullContainer.push_back(index)
		if len(_container) <= index:
			_container.resize(index + 1)
		_container[index] = null

func get_null():
	if _nullContainer.size() > 1:
		return _nullContainer[_count]

func exists(index):
	if _container[index] == null:
		return false
	return true
	
func clear():
	_container.clear()
	_nullContainer.clear()
	_count = 0
	
func count():
	return _count -1
#endregion NullList
